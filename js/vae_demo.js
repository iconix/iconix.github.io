  ...
  var vae_sketch = function( p ) {
    ...
    function draw_sliders() {
      ...
      for(i=0;i<N;i++) {
        ...
        slider[i].position(slider_x[i], origy+slider_y[i])
      }
    }
    ...
    p.draw = function() {
      ...
      if (loading_sent && train_sent && recon_sent) {
        ...
        p.text(train_sent, train_x, train_y, textbox_size, textbox_size)

        p.textSize(text_size)
        p.text(recon_sent, recon_x, recon_y, textbox_size, textbox_size)
      }
    }
